...
namespace DUR.Api.Services.Services
{
    public class AppointmentResponseService : DatabaseServiceBase<AppointmentResponse>, IAppointmentResponseService
    {
        ...
        public AppointmentResponseService(IDatabaseUnitOfWorkFactory unitOfWorkFactory, IApplicationLogger logger, IGroupMailService groupMailService) : base(logger)
        {
        ...
        public bool SignOffForAppointment(AppointmentResponse response)
        {
            ...
            if (success)
            {
                success = _groupMailService.SignOff(response);
            }
            return success;
        }
        ...
        public bool SignOnForAppointment(AppointmentResponse response)
        {
            ...
            if (success)
            {
                success = _groupMailService.SignOn(response);
            }
            return success;
        }
    }

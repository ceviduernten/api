...
public class ContactPresenter : BasePresenter<ContactRM, Contact>, IContactPresenter
{
    ...
    public ContactGroupRM GetContacts()
    {
        ...
        var contactGroup = new ContactGroupRM();
        contactGroup.Jungschar = allContacts.Where(x => x.Type == ContactType.JUNGSCHAR).OrderByDescending(x => x.Phone)
            .ToList();
        contactGroup.Froeschli = allContacts.Where(x => x.Type == ContactType.FROESCHLI).OrderByDescending(x => x.Phone)
            .ToList();
        contactGroup.Verein = allContacts.Where(x => x.Type == ContactType.VEREIN).OrderByDescending(x => x.Phone)
            .ToList();
        return contactGroup;
    }

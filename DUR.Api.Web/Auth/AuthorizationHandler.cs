...
public class AuthorizationHandler : AuthorizationHandler<AuthorizationRequirement>
{
        ...
        AuthorizationRequirement requirement)
    {
        ...
        var currentRole =
            (Role) Enum.Parse(typeof(Role), context.User.Claims.First(c => c.Type == ClaimTypes.Role).Value);
        ...
        if (requirement.Permission == "Admin")
        {
            if (isEnumValue && currentRole == Role.ADMIN) context.Succeed(requirement);
            return Task.CompletedTask;
        }

        if (requirement.Permission == "Stuff")
        {
            if (isEnumValue && (currentRole == Role.ADMIN || currentRole == Role.STUFF_LEADER))
                context.Succeed(requirement);
            return Task.CompletedTask;
        }

        if (requirement.Permission == "Scouting")
        {
            if (isEnumValue && (currentRole == Role.ADMIN || currentRole == Role.STUFF_LEADER ||
                                currentRole == Role.SCOUTING_LEADER)) context.Succeed(requirement);
            return Task.CompletedTask;
        }
